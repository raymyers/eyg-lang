...
pub fn expr_to_string(expr: Expr) -> String {
  case expr {
    ...
    Grouping(expression, _) -> "(group " <> expr_to_string(expression) <> ")"
    ...
    Match(value, cases, _) -> {
      let cases_str =
        ...
        |> list.map(fn(case_) {
          "(case (pattern "
          <> expr_to_string(case_.pattern)
          <> ") "
          <> expr_to_string(case_.body)
          <> ")"
        })
        |> string.join(" ")
    ...
    Handle(effect, handler, fallback, _) ->
      ...
      <> effect
      <> " "
      <> expr_to_string(handler)
      <> " "
      <> expr_to_string(fallback)
      <> ")"
    ...
    Thunk(body, _) -> "(thunk " <> expr_to_string(body) <> ")"

    Spread(expression, _) -> "(spread " <> expr_to_string(expression) <> ")"
...
fn value_to_string(value: Value) -> String {
  case value {
    ...
    UnionValue(constructor, val) ->
      constructor <> "(" <> value_to_string(val) <> ")"
    RecordValue(fields) -> {
      let fields_str =
        fields
    ...
    ErrorValue(message, line) ->
      "Error(" <> message <> " at line " <> int.to_string(line) <> ")"
  }
}
