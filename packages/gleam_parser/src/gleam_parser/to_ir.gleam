...
pub fn to_annotated(expr: ast.Expr, rev: List(Int)) -> #(ir.Expression(List(Int)), List(Int)) {
  case expr {
    // Basic expressions
    ...
    ast.Literal(value, _) -> {
      case value {
        ...
        _ -> #(ir.Vacant, rev) // Fallback for other value types
      }
    }
    ...
    ast.Binary(left, operator, right, _) -> {
      ...
      #(ir.Apply(#(ir.Apply(#(ir.Builtin(builtin_name), rev), left_ir), rev), right_ir), rev)
    }
    ...
    ast.Record(fields, _) -> {
      ...
      let #(_, result) = list.fold_right(fields, #(len, empty_record), fn(acc, field) {
        let #(i, acc_ir) = acc
        let i = i - 2
        let field_value = to_annotated(field.value, [i + 1, ..rev])
        let extended = #(
          ir.Apply(
            #(ir.Apply(#(ir.Extend(field.name), [i, ..rev]), field_value), rev),
            acc_ir
          ),
          rev
        )
    ...
    ast.List(elements, _) -> {
      ...
      let #(_, result) = list.fold_right(elements, #(len, tail_ir), fn(acc, element) {
        let #(i, acc_ir) = acc
        let i = i - 1
        let element_ir = to_annotated(element, [i, ..rev])
        let consed = #(ir.Apply(#(ir.Apply(#(ir.Cons, rev), element_ir), rev), acc_ir), rev)
        #(i, consed)
      })
      ...
      case pattern {
        ...
        ast.Destructure(fields, _) -> {
          ...
          list.fold_right(fields, body_ir, fn(acc, field) {
            let select_ir = #(ir.Apply(
              #(ir.Select(field.name), rev),
              #(ir.Variable("$destructure"), rev)
            ), rev)
    ...
    ast.Block(statements, _) -> {
      case statements {
        ...
        _ -> {
          ...
          list.index_fold_right(init_stmts, last_ir, fn(acc, stmt, i) {
    ...
    ast.Match(value, cases, _) -> {
      ...
      let #(_, cases_ir) = list.fold_right(cases, #(len + 1, nocases), fn(acc, case_) {
        let #(i, acc_ir) = acc
        let i = i - 1
    ...
    ast.Destructure(fields, _) -> {
      ...
      let field_exprs = list.map(fields, fn(field) {
        let var_name = case field.value {
          ast.Variable(name, _) -> name
          _ -> "_unknown"
        }
